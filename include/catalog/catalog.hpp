...
/**
 * This component manages all metadata for databases. There is lower component
 * DatabaseCatalog that manages tables, schemas... Its implemented using
 * relational model and all operations inside it should work just like any other
 * table operation. Meaning its fully transactional and recoverable in case of
 * errors. There is only a single catalog instance at a time.
 */
class Catalog {
private:
...
public:
  ...
  /**
   * Creates bunch of systems table and a database catalog instance that manages
   * them. Inserts and entry to the pg_database table that tracks database
   * instances.
   * @param txn transaction context that manages transaction operations.
   * @param name database name
