...
namespace db7::access
{
    ...
    private:
        ...
        u32 AppendHeap(byte *data, Key key, R value)
        {
            ...
            u32 off = shared::AlignDown(PAGE_SIZE - heap_size - key.enc_len - sizeof(SlotValHeader<R>), alignof(SlotValHeader<R>));
        ...
        void CopyRange(byte *from, byte *to, Slot *from_slots, u32 start, u32 end)
        {
            ...
            to_slots[0] = Slot{AppendHeap(to, Key{}, val.hdr.result)};

            for (u32 i = start + 1; i < end; i++)
            {
                SlotVal val = CastSlot(ReadSlot(from, from_slots[i]));
                u32 off = AppendHeap(to, MakeEncodedKey(val.hdr.len, val.data), val.hdr.result);
        ...
        {
            byte *sentinel_copy = new byte[key.enc_len];
            std::memcpy(sentinel_copy, key.encoded, key.enc_len);
            return MakeEncodedKey(key.enc_len, sentinel_copy);
        }
    ...
    public:
        ...
        Key Split(byte *left_data, byte *right_data, page_id new_pid, Key key, R value)
        {
            ...
            if (ReadMaxVal(left_header) != UNDEFINED)
            {
                ...
                right_max = (u64)AppendHeap(right_data, MakeEncodedKey(val.hdr.len, val.data), val.hdr.result);
            }
