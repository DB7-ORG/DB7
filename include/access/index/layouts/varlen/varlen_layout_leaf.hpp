...
template <typename R> struct SlotValLeaf {
  ...
  SlotValLeaf(byte *page_data, u16 heap_offset)
      : len(*reinterpret_cast<u16 *>(page_data + heap_offset)),
...
private:
  ...
  int FindInsertPosition(byte *data, u16 *slots, SlotValLeaf<ValTyp> main_val,
    ...
    while (lo < hi) {
      ...
      // NOTE: this war removed since im doing optimistic reading which with an
      // active writer can produce any of the results but its fine since the
      // thread will retry later DB7_ASSERT_FMT(res != 0, "duplicate (key,tid):
      // mid={} count={} len={} tid={}",
      //                mid, count, main_val.len, (unsigned long
      //                long)main_val.Result());
      if (res < 0)
        lo = mid + 1;
      else
        hi = mid;
  ...
  bool HighPrefixCmp(byte *data, Key key) {
    ...
    // DB7_ASSERT(cmp != 0, "Can not have value to be 0 when inserting the
    // tree");
    return cmp == 0;
  }
...
public:
  ...
  ResultObj<void> Get(byte *data, const u16 count, const Key key,
  ...
  ResultObj<void> Delete(byte *data, Key key) {
    ...
    DB7_ASSERT(cmp == 0, "Unreachable. Key is not found. This should only be "
                         "called by GC. GC tried to delete non existing value");
