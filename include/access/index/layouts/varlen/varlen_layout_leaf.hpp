...
#define DB7_MAX_SLOTS_PER_PAGE (DB7_PAGE_SIZE - sizeof(VarlenHeader<ValTyp>)) / sizeof(u32)
    ...
    private:
        ...
        u32 ReserveSlot(byte *data, u32 len)
        {
            ...
            u32 off = shared::AlignDown(DB7_PAGE_SIZE - heap_size - len - sizeof(SlotValHeader<ValTyp>), alignof(SlotValHeader<ValTyp>));
        ...
        void CompactHeap(byte *data, Slot *slots, u32 count, u32 prefix_len)
        {
            ...
            for (u32 i = 0; i < count; i++)
            {
                ...
                u32 off = shared::AlignDown(DB7_PAGE_SIZE - write_pos - entry_size, alignof(SlotValHeader<ValTyp>));
        ...
        ResultObj<void> DeleteInternal(byte *data, Key key)
        {
            ...
            header->dead_space += shared::AlignUp((sizeof(SlotValHeader<ValTyp>) + hdr->len), alignof(SlotValHeader<ValTyp>));
