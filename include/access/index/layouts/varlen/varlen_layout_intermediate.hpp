...
template <typename R> struct SlotValInter {
  ...
  SlotValInter(byte *page_data, u16 heap_offset)
      : hdr(*reinterpret_cast<SlotValHeaderInter<R> *>(page_data +
                                                       heap_offset)),
        data(page_data + heap_offset + sizeof(SlotValHeaderInter<R>)) {}

  // NOTE: this one threats value(result) as 0 so it navigates to the leftmost
  // value
...
private:
  ...
  u16 CalculateHeapOffset(u16 prev_heap_offset, u16 tuple_len) {
    u16 heap_offset =
        prev_heap_offset - tuple_len - sizeof(SlotValHeaderInter<ValTyp>);
  ...
  u16 AppendHeap(byte *data, Key key, ValTyp value) {
    ...
    /* Fill header */
    auto *hdr =
        reinterpret_cast<SlotValHeaderInter<ValTyp> *>(data + new_heap_offset);
    *hdr = {value, key.len};
    /* Fill value */
    std::memcpy(data + new_heap_offset + sizeof(SlotValHeaderInter<ValTyp>),
                key.data, key.len);
  ...
  u16 MoveHeap(byte *data, u16 old_offset, u16 len) {
    ...
    /* Fill value */
    std::memmove(data + new_heap_offset, data + old_offset,
                 sizeof(SlotValHeaderInter<ValTyp>) + len);
    /* Update heap offset to point to new value */
    prev_heap_offset = new_heap_offset;
   ...
   * positive → slot > main
   */
  inline int CmpFull(SlotValInter<ValTyp> slot_val,
                     SlotValInter<ValTyp> main_val) {
    ...
    return (main_val.hdr.len < slot_val.hdr.len) -
           (main_val.hdr.len > slot_val.hdr.len);
  }

  int FindInsertPosition(byte *data, u16 *slots, SlotValInter<ValTyp> main_val,
                         u16 count) {
    int hi = count, lo = 0;
    while (lo < hi) {
      int mid = lo + (hi - lo) / 2;
      ...
      DB7_ASSERT(res != 0, "Somehow there are identical entries in the tree. "
                           "That means same tuple was inserted twice");
      if (res < 0)
        lo = mid + 1;
      else
        hi = mid;
  ...
  int FindChild(byte *data, u16 *slots, SlotValInter<ValTyp> main_val,
                u16 count) {
    int lo = 0, hi = count;
    while (lo < hi) {
      int mid = lo + (hi - lo) / 2;
  ...
  u16 CopyHalfRight(byte *__restrict left_data, byte *__restrict right_data,
    ...
    if (left_header->max_val != UNDEFINED_OFFSET) {
      ...
      right_max =
          AppendHeap(right_data, {slot.hdr.len, slot.data}, slot.hdr.result);
    }
  ...
  void CompactHeap(byte *data, u32 count) {
    DB7_ASSERT(VarlenHeader::CastHeader(data)->heap_offset == DB7_PAGE_SIZE,
               "heap not set correctly");
    u16 *slots = CastSlots(data);
    ...
    for (u32 i = 0; i < count; i++) {
      u16 idx = indexes[i];
      auto *hdr =
          reinterpret_cast<SlotValHeaderInter<ValTyp> *>(data + slots[idx]);
  ...
  u16 FindSplitPoint(byte *data, u16 *slots, u16 count) {
    ...
    for (int i = 0; i < count; i++)
      live +=
          reinterpret_cast<SlotValHeaderInter<ValTyp> *>(data + slots[i])->len;
...
public:
  ...
  ValTyp Get(byte *data, const u32 count, const Key key) {
    ...
    DB7_ASSERT(idx >= 0,
               "no covering child — leftmost separator must be the empty key");
    return reinterpret_cast<SlotValHeaderInter<ValTyp> *>(data + slots[idx - 1])
        ->result;
  }
   ...
   */
  Key Split(byte *__restrict left_data, byte *__restrict right_data,
            ValTyp new_pid, Key key, ValTyp value) {
    DB7_ASSERT(key.data != nullptr, "invalid key");
    DB7_ASSERT(key.len != 0, "invalid key");
    ...
    u16 left_max = AppendHeap(left_data, {sentinel.hdr.len, sentinel.data},
                              sentinel.hdr.result);
