 ...
 * Used to implement abstractions that hide whether its working with row store
 * or column store. Idea is for this layer when its working in catalog to act as
 * a row store and when its working in excecution to operate as column store
 */
namespace db7::access {
enum class RowStatus { Visible, Deleted, NotVisible };
/**
 * Table abstraction
 */
class Table {
