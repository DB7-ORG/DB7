...
public:
  ...
  template <typename T> static u32 Encode(byte *buf, T data, bool is_data_null, KeySpecs specs) {
    ...
    if constexpr (std::is_integral_v<T> && std::is_signed_v<T>) { // i32, i64...
      ...
      size += sizeof(T);
    } else if constexpr (std::is_unsigned_v<T>) { // u32, u64...
      ...
      size += sizeof(T);
                         ...
                         std::is_same_v<T, std::span<byte>>) { // strings, bytes
                                                               // ...
  ...
  static Key BuildKey(byte *out, DataChunk *chunk, u64 key_value, std::vector<TypeSize> &types) {
    ...
    // NOTE: i removed original value since it only makes sense for index only
    // scans which are rare might add it later if needed fr now keep it simple
    // for (size_t i = 0; i < types.size(); i++)
    // {
