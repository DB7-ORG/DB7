...
public:
  ...
  auto begin() { return std::views::values(mapping_).begin(); }

  auto end() { return std::views::values(mapping_).end(); }

  auto begin() const { return std::views::values(mapping_).begin(); }

  auto end() const { return std::views::values(mapping_).end(); };
};
} // namespace db7::access