...
[[noreturn]] static void ThrowUnsupported(const char *fn, const char *what,
                                          int value) {
...
PostgresParser::BuildParseTree(const std::string &query_string) {
  ...
  if (result.stderr_buffer != nullptr) {
  ...
  if (result.error != nullptr) {
    ...
    ParserException exception(std::string(result.error->message), __FILE__,
                              __LINE__, result.error->cursorpos);
...
PostgresParser::NodeTransform(ParseResult *parse_result, Node *node) {
  ...
  case T_CreateStmt: {
    result =
        CreateTransform(parse_result, reinterpret_cast<CreateStmt *>(node));
    break;
  }
  case T_CreatedbStmt: {
    result = CreateDatabaseTransform(parse_result,
                                     reinterpret_cast<CreatedbStmt *>(node));
    break;
  }
  case T_CreateFunctionStmt: {
    result = CreateFunctionTransform(
        parse_result, reinterpret_cast<CreateFunctionStmt *>(node));
    break;
  }
  case T_CreateSchemaStmt: {
    result = CreateSchemaTransform(parse_result,
                                   reinterpret_cast<CreateSchemaStmt *>(node));
    break;
  }
  case T_CreateTrigStmt: {
    result = CreateTriggerTransform(parse_result,
                                    reinterpret_cast<CreateTrigStmt *>(node));
    break;
  }
  // case T_DropdbStmt: {
  ...
  case T_IndexStmt: {
    result =
        CreateIndexTransform(parse_result, reinterpret_cast<IndexStmt *>(node));
    break;
  }
  // case T_InsertStmt: {
  //   result =
  ...
  case T_SelectStmt: {
    result =
        SelectTransform(parse_result, reinterpret_cast<SelectStmt *>(node));
    break;
  }
  // case T_VacuumStmt: {
  //   result =
  ...
  case T_ViewStmt: {
    result =
        CreateViewTransform(parse_result, reinterpret_cast<ViewStmt *>(node));
    break;
  }
  // case T_TruncateStmt: {
  //   result =
...
PostgresParser::ExprTransform(ParseResult *parse_result, Node *node,
                              char *alias) {
  if (node == nullptr) {
    return nullptr;
  }
  ...
  case T_ColumnRef: {
    expr = ColumnRefTransform(parse_result, reinterpret_cast<ColumnRef *>(node),
                              alias);
    break;
  }
  case T_FuncCall: {
  ...
  case T_SubLink: {
    expr =
        SubqueryExprTransform(parse_result, reinterpret_cast<SubLink *>(node));
    break;
  }
  case T_TypeCast: {
    expr = AExprTransform(parse_result, reinterpret_cast<A_Expr *>(node));
  ...
  case T_Integer: {
    expr = std::make_unique<ConstantValueExpression>(access::type_id::INTEGER,
                                                     Integer(PgIntVal(node)));
    break;
  }
  default: {
  ...
  if (alias != nullptr) {
    expr->SetAlias(
        parser::AliasType(alias, alias_oid_t(reinterpret_cast<size_t>(
                                     reinterpret_cast<void *>(expr.get())))));
  }
  return expr;
}
 ...
 */
ExpressionType
PostgresParser::StringToExpressionType(const std::string &parser_str) {
...
std::unique_ptr<AbstractExpression>
PostgresParser::AExprTransform(ParseResult *parse_result, A_Expr *root) {
  if (root == nullptr) {
    return nullptr;
  }
  ...
  if (root->kind == AEXPR_DISTINCT) {
    ...
    if (std::strcmp(name, "<>") == 0) {
      auto in_expr = std::make_unique<ComparisonExpression>(
          target_type, std::move(children));
      std::vector<std::unique_ptr<AbstractExpression>> in_child;
      in_child.emplace_back(std::move(in_expr));
      return std::make_unique<OperatorExpression>(ExpressionType::OPERATOR_NOT,
  ...
  case ExpressionType::OPERATOR_EXISTS: {
    return std::make_unique<OperatorExpression>(
        target_type, access::type_id::INVALID, std::move(children));
  }
  ...
  default: {
    fmt::print("AExprTransform: type {} unsupported",
...
std::unique_ptr<AbstractExpression>
PostgresParser::BoolExprTransform(ParseResult *parse_result, BoolExpr *root) {
  ...
  case NOT_EXPR: {
    result = std::make_unique<OperatorExpression>(ExpressionType::OPERATOR_NOT,
                                                  access::type_id::INVALID,
                                                  std::move(children));
    break;
  }
...
std::unique_ptr<AbstractExpression>
PostgresParser::CaseExprTransform(ParseResult *parse_result, CaseExpr *root) {
  if (root == nullptr) {
    return nullptr;
  }

  auto arg_expr =
      ExprTransform(parse_result, reinterpret_cast<Node *>(root->arg), nullptr);

  std::vector<CaseExpression::WhenClause> clauses;
  for (int i = 0; i < list_length(root->args); i++) {
    auto *w = static_cast<CaseWhen *>(list_nth(root->args, i));
    auto when_expr =
        ExprTransform(parse_result, reinterpret_cast<Node *>(w->expr), nullptr);
    auto result_expr = ExprTransform(
        parse_result, reinterpret_cast<Node *>(w->result), nullptr);
  ...
  auto default_expr = ExprTransform(
      parse_result, reinterpret_cast<Node *>(root->defresult), nullptr);
...
std::unique_ptr<AbstractExpression>
PostgresParser::ColumnRefTransform(ParseResult *parse_result, ColumnRef *root,
                                   char *alias) {
  std::unique_ptr<AbstractExpression> result;
  ...
  case T_String: {
    ...
    if (all_columns)
       ...
       * a matching serial number
       */
      result = std::make_unique<ColumnValueExpression>(AliasType(table_name),
                                                       col_name);
    break;
  }
  case T_A_Star: {
...
std::unique_ptr<AbstractExpression>
PostgresParser::ConstTransform(ParseResult *parse_result, A_Const *root) {
  if (root == nullptr) {
    return nullptr;
  }
...
std::unique_ptr<AbstractExpression>
PostgresParser::FuncCallTransform(ParseResult *parse_result, FuncCall *root) {
  ...
  if (!IsAggregateFunction(func_name)) {
    ...
    result = std::make_unique<FunctionExpression>(
        std::move(func_name), access::type_id::INVALID, std::move(children));
  } else {
    // aggregate function
    ...
    if (root->agg_star) {
      ...
      fmt::print(
          "FuncCallTransform: aggregate must have exactly one argument\n");
...
std::unique_ptr<AbstractExpression>
PostgresParser::NullTestTransform(ParseResult *parse_result, NullTest *root) {
  if (root == nullptr) {
    return nullptr;
  }

  std::vector<std::unique_ptr<AbstractExpression>> children;
  children.emplace_back(ExprTransform(
      parse_result, reinterpret_cast<Node *>(root->arg), nullptr));

  ExpressionType type = root->nulltesttype == IS_NULL
                            ? ExpressionType::OPERATOR_IS_NULL
                            : ExpressionType::OPERATOR_IS_NOT_NULL;

  return std::make_unique<OperatorExpression>(type, access::type_id::BOOLEAN,
                                              std::move(children));
}

// Postgres.ParamRef -> noisepage.ParameterValueExpression
std::unique_ptr<AbstractExpression>
PostgresParser::ParamRefTransform(ParseResult *parse_result, ParamRef *root) {
...
// OperatorExpression
std::unique_ptr<AbstractExpression>
PostgresParser::SubqueryExprTransform(ParseResult *parse_result,
                                      SubLink *node) {
  if (node == nullptr) {
    return nullptr;
  }

  auto select_stmt = SelectTransform(
      parse_result, reinterpret_cast<SelectStmt *>(node->subselect));
  auto subquery_expr =
  ...
  case ANY_SUBLINK: {
    ...
    if (node->operName != nullptr &&
        std::strcmp(strVal(llast(node->operName)), "=") != 0) {
      throw PARSER_EXCEPTION(
          "SubqueryExprTransform: only IN and = ANY are supported");
    }
    ...
    result = std::make_unique<ComparisonExpression>(ExpressionType::COMPARE_IN,
                                                    std::move(children));
    break;
  }
  case EXISTS_SUBLINK: {
    ...
    result = std::make_unique<OperatorExpression>(
        ExpressionType::OPERATOR_EXISTS, access::type_id::BOOLEAN,
        std::move(children));
    break;
  }
  case EXPR_SUBLINK: {
...
std::unique_ptr<AbstractExpression>
PostgresParser::TypeCastTransform(ParseResult *parse_result, TypeCast *root) {
...
// Postgres.A_Const -> noisepage.ConstantValueExpression
std::unique_ptr<AbstractExpression>
PostgresParser::ValueTransform(ParseResult *parse_result, A_Const *root) {
  // NULL literals no longer have their own node type
  if (root->isnull) {
    return std::make_unique<ConstantValueExpression>(access::type_id::INVALID,
                                                     Val(true));
  }
  ...
  case T_Integer: {
    return std::make_unique<ConstantValueExpression>(
        access::type_id::INTEGER, Integer(PgIntVal(&root->val)));
  }

  case T_String: {
    auto string_val =
        ValueUtil::CreateStringVal(std::string_view{strVal(&root->val)});
    return std::make_unique<ConstantValueExpression>(
        access::type_id::VARCHAR, string_val.first,
  ...
  case T_Float: {
    ...
    if (std::strpbrk(str, ".eE") == nullptr) {
      return std::make_unique<ConstantValueExpression>(
          access::type_id::BIGINT, Integer(std::stoll(str)));
    }
    return std::make_unique<ConstantValueExpression>(access::type_id::DOUBLE,
                                                     Real(std::stod(str)));
  }
...
PostgresParser::SelectTransform(ParseResult *parse_result, SelectStmt *root) {
  ...
  case SETOP_NONE: {
    ...
    auto groupby =
        GroupByTransform(parse_result, root->groupClause, root->havingClause);
    ...
    result = std::make_unique<SelectStatement>(
        std::move(target), select_distinct, std::move(from), where,
...
PostgresParser::FromItemTransform(ParseResult *parse_result, Node *node) {
  switch (nodeTag(node)) {
  case T_RangeVar:
    return RangeVarTransform(parse_result, reinterpret_cast<RangeVar *>(node));
  case T_RangeSubselect:
    return RangeSubselectTransform(parse_result,
  ...
  default:
    ThrowUnsupported("FromItemTransform", "FROM item type",
                     static_cast<int>(nodeTag(node)));
  }
}
...
std::unique_ptr<TableRef>
PostgresParser::FromTransform(ParseResult *parse_result,
  ...
  for (int i = 0; i < list_length(root); i++) {
    refs.emplace_back(FromItemTransform(
        parse_result, static_cast<Node *>(list_nth(root, i))));
  }
...
PostgresParser::GroupByTransform(ParseResult *parse_result, List *group,
                                 Node *having_node) {
...
std::unique_ptr<OrderByDescription>
PostgresParser::OrderByTransform(ParseResult *parse_result, List *order) {
  if (order == nullptr) {
    return nullptr;
  }
  ...
  for (int i = 0; i < list_length(order); i++) {
    ...
    if (nodeTag(temp) != T_SortBy) {
      ThrowUnsupported("OrderByTransform", "ORDER BY item type",
                       static_cast<int>(nodeTag(temp)));
    }
    ...
    default: // SORTBY_USING
      ThrowUnsupported("OrderByTransform", "sort direction",
                       static_cast<int>(sort->sortby_dir));
    }
...
PostgresParser::WhereTransform(ParseResult *parse_result, Node *root) {
  if (root == nullptr) {
    return nullptr;
  }
...
std::unique_ptr<JoinDefinition>
PostgresParser::JoinTransform(ParseResult *parse_result, JoinExpr *root) {
  if (root->isNatural) {
    throw PARSER_EXCEPTION("JoinTransform: NATURAL JOIN is not supported");
  }
  ...
  case JOIN_SEMI:
    ...
    break;
  default:
    ThrowUnsupported("JoinTransform", "join type",
  ...
  if (root->quals == nullptr) {
    throw PARSER_EXCEPTION(
        "JoinTransform: CROSS JOIN and JOIN ... USING are not supported");
  }
  ...
  return std::make_unique<JoinDefinition>(type, std::move(left),
                                          std::move(right), condition);
}
...
std::unique_ptr<TableRef>
PostgresParser::RangeVarTransform(ParseResult *parse_result, RangeVar *root) {
...
std::unique_ptr<TableRef>
PostgresParser::RangeSubselectTransform(ParseResult *parse_result,
                                        RangeSubselect *root) {
  auto select = SelectTransform(parse_result,
                                reinterpret_cast<SelectStmt *>(root->subquery));
  if (select == nullptr) {
    return nullptr;
  }
  auto alias = AliasTransform(root->alias);
...
static std::unique_ptr<TableInfo> MakeTableInfo(const RangeVar *rv) {
  return std::make_unique<TableInfo>(
      OrEmpty(rv->relname), OrEmpty(rv->schemaname), OrEmpty(rv->catalogname));
}
...
static std::vector<std::string> StringList(const List *list) {
  ...
  for (int i = 0; i < list_length(list); i++) {
    out.emplace_back(PgStrVal(list_nth(list, i)));
  }
  return out;
}
...
static const char *DefElemString(const DefElem *def) {
  if (def->arg == nullptr || nodeTag(def->arg) != T_String) {
    throw PARSER_EXCEPTION(
        fmt::format("option \"{}\" expects a string value", def->defname));
  }
...
static BaseFunctionParameter::DataType
FuncDataTypeFromPgName(const char *name) {
  std::string_view n{name};
  if (n == "int" || n == "int4")
    return BaseFunctionParameter::DataType::INT;
  if (n == "varchar")
    return BaseFunctionParameter::DataType::VARCHAR;
  if (n == "int8")
    return BaseFunctionParameter::DataType::BIGINT;
  if (n == "int2")
    return BaseFunctionParameter::DataType::SMALLINT;
  if (n == "double" || n == "float8")
    return BaseFunctionParameter::DataType::DOUBLE;
  if (n == "real" || n == "float4")
    return BaseFunctionParameter::DataType::FLOAT;
  if (n == "text")
    return BaseFunctionParameter::DataType::TEXT;
  if (n == "bpchar")
    return BaseFunctionParameter::DataType::CHAR;
  if (n == "tinyint")
    return BaseFunctionParameter::DataType::TINYINT;
  if (n == "bool")
    return BaseFunctionParameter::DataType::BOOL;
  throw PARSER_EXCEPTION(
      fmt::format("unsupported function data type \"{}\"", name));
}
...
std::unique_ptr<SQLStatement>
PostgresParser::CreateTransform(ParseResult *parse_result, CreateStmt *root) {
  ...
  for (int i = 0; i < list_length(root->tableElts); i++) {
    ...
    case T_ColumnDef: {
      auto res =
          ColumnDefTransform(parse_result, reinterpret_cast<ColumnDef *>(node));
      columns.emplace_back(std::move(res.col_));
      foreign_keys.insert(foreign_keys.end(),
                          std::make_move_iterator(res.fks_.begin()),
    ...
    case T_Constraint: {
      ...
      case CONSTR_PRIMARY: {
        for (auto &key : StringList(constraint->keys)) {
          primary_keys.emplace(std::move(key));
        }
        break;
      }
      case CONSTR_FOREIGN: {
...
std::unique_ptr<SQLStatement>
PostgresParser::CreateDatabaseTransform(ParseResult *parse_result,
                                        CreatedbStmt *root) {
...
std::unique_ptr<SQLStatement>
PostgresParser::CreateFunctionTransform(ParseResult *parse_result,
                                        CreateFunctionStmt *root) {
  if (root->is_procedure) {
    throw PARSER_EXCEPTION("CREATE PROCEDURE is not supported");
  }
  ...
  for (int i = 0; i < list_length(root->parameters); i++) {
    ...
    if (nodeTag(node) == T_FunctionParameter) {
      func_parameters.emplace_back(FunctionParameterTransform(
          parse_result, reinterpret_cast<FunctionParameter *>(node)));
    }
  ...
  for (int i = 0; i < list_length(root->options); i++) {
    ...
    if (std::strcmp(def_elem->defname, "as") == 0) {
      ...
      as_type =
          func_body.size() > 1 ? AsType::EXECUTABLE : AsType::QUERY_STRING;
      ...
      if (std::strcmp(lang, "plpgsql") == 0) {
        ...
        throw PARSER_EXCEPTION(fmt::format(
            "CreateFunctionTransform: language \"{}\" not supported", lang));
      }
    }
  }

  return std::make_unique<CreateFunctionStatement>(
...
std::unique_ptr<SQLStatement>
PostgresParser::CreateIndexTransform(ParseResult *parse_result,
                                     IndexStmt *root) {
  auto unique = root->unique;

  assert(root->relation->relname != nullptr &&
         "It can't be empty. See postgres spec.");
  ...
  for (int i = 0; i < list_length(root->indexParams); i++) {
    ...
    if (index_elem->expr == nullptr) {
      ...
      if (no_name) {
        index_name += "_" + std::string(index_elem->name);
      }
    } else {
  ...
  return std::make_unique<CreateStatement>(std::move(table_info), index_type,
                                           unique, index_name,
...
std::unique_ptr<SQLStatement>
PostgresParser::CreateSchemaTransform(ParseResult *parse_result,
...
std::unique_ptr<SQLStatement>
PostgresParser::CreateTriggerTransform(ParseResult *parse_result,
                                       CreateTrigStmt *root) {
  auto table_info = MakeTableInfo(root->relation);
...
std::unique_ptr<SQLStatement>
PostgresParser::CreateViewTransform(ParseResult *parse_result, ViewStmt *root) {
  ...
  auto view_query = SelectTransform(
      parse_result, reinterpret_cast<SelectStmt *>(root->query));

  return std::make_unique<CreateStatement>(view_name, std::move(view_query));
}

// Postgres.ColumnDef -> noisepage.ColumnDefinition
PostgresParser::ColumnDefTransResult
PostgresParser::ColumnDefTransform(ParseResult *parse_result, ColumnDef *root) {
  ...
  if (type_name->typmods != nullptr) {
    ...
    if (nodeTag(node) != T_A_Const) {
      ThrowUnsupported("ColumnDefTransform", "type modifier",
                       static_cast<int>(nodeTag(node)));
    }
    auto *c = reinterpret_cast<A_Const *>(node);
  ...
  for (int i = 0; i < list_length(root->constraints); i++) {
    ...
    if (nodeTag(node) != T_Constraint) {
      ThrowUnsupported("ColumnDefTransform", "column option",
                       static_cast<int>(nodeTag(node)));
    }
    auto *constraint = reinterpret_cast<Constraint *>(node);
    ...
    default:
      ThrowUnsupported("ColumnDefTransform", "constraint",
                       static_cast<int>(constraint->contype));
    }
  }

  auto result = std::make_unique<ColumnDefinition>(
      root->colname, datatype, is_primary, is_not_null, is_unique, default_expr,
      check_expr, varlen);
...
std::unique_ptr<FuncParameter>
PostgresParser::FunctionParameterTransform(ParseResult *parse_result,
                                           FunctionParameter *root) {
  auto data_type =
      FuncDataTypeFromPgName(PgStrVal(llast(root->argType->names)));
...
PostgresParser::ReturnTypeTransform(ParseResult *parse_result, TypeName *root) {
...
std::unique_ptr<AbstractExpression>
PostgresParser::WhenTransform(ParseResult *parse_result, Node *root) {
...
PostgresParser::ColumnNameTransform(List *root) {
...
std::vector<std::unique_ptr<TableRef>>
PostgresParser::WithTransform(ParseResult *parse_result, WithClause *root) {
  ...
  for (int i = 0; i < list_length(root->ctes); i++) {
    ...
    if (cte->ctequery == nullptr || nodeTag(cte->ctequery) != T_SelectStmt) {
      ThrowUnsupported("WithTransform", "CTE query type",
                       cte->ctequery ? static_cast<int>(nodeTag(cte->ctequery))
                                     : -1);
    }
    ...
    for (int j = 0; j < list_length(cte->aliascolnames); j++) {
      colnames.emplace_back(
          AliasType(std::string(PgStrVal(list_nth(cte->aliascolnames, j))),
    ...
    const CteType cte_type =
        root->recursive
            ? (select->HasUnionSelect() ? CteType::STRUCTURALLY_RECURSIVE
                                        : CteType::RECURSIVE)
            : CteType::SIMPLE;

    ctes.push_back(TableRef::CreateCTETableRefBySelect(
        cte->ctename, std::move(select), std::move(colnames), cte_type));
  }
  return ctes;
}
