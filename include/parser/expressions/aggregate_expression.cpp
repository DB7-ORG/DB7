...
std::unique_ptr<AbstractExpression> AggregateExpression::CopyWithChildren(
  ...
  auto expr = std::make_unique<AggregateExpression>(
      GetExpressionType(), std::move(children), IsDistinct());
...
void AggregateExpression::DeriveReturnValueType() {
  ...
  case ExpressionType::AGGREGATE_COUNT:
    this->SetReturnValueType(access::type_id::INTEGER);
    break;
  // keep the type of the base
  case ExpressionType::AGGREGATE_MAX:
  case ExpressionType::AGGREGATE_MIN:
  case ExpressionType::AGGREGATE_SUM:
    assert(this->GetChildrenSize() >= 1 && "No column name given.");
    const_cast<parser::AbstractExpression *>(this->GetChild(0).Get())
        ->DeriveReturnValueType();
    this->SetReturnValueType(this->GetChild(0)->GetReturnValueType());
    break;
  case ExpressionType::AGGREGATE_AVG:
    this->SetReturnValueType(access::type_id::DOUBLE);
    break;
...
AggregateExpression::FromJson(const nlohmann::json &j) {
...
hash_t AggregateExpression::Hash() const {
  ...
  hash =
      shared::HashUtil::CombineHashes(hash, shared::HashUtil::Hash(distinct_));
  return hash;
}
