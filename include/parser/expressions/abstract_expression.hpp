...
/**
 * Type to represent aliases. This was added to account for the fact that you
 * can have duplicate aliases in queries that still need to have distinguishable
 * identifiers. For example in SELECT * FROM (SELECT 1 as x, 2 as x) p; We
 * achieve this by having an additional serial number field. This field allows
 * for two aliases with the same name string but different valid serial numbers
 * to be distinguishable. However, we maintain that an alias with name "xxx" but
 * with an invalid serial number matches with all aliases with name "xxx"
 * regardless of their serial number
 */
class AliasType {
private:
  std::string name_;
...
public:
   ...
   * @param name Alias name
   */
  explicit AliasType(const std::string &name)
      : name_{std::string(name)}, serial_no_{0}, serial_valid_{false} {}
   ...
   * @return Whether or not these two aliases are considered equal as documented
   * above
   */
   ...
   * Not Equals function
   * @param other The alias we are comparing against
   * @return Whether or not these two aliases are considered not equal as
   * documented above
   */
  ...
  /**
   * Compare function struct for comparing aliases purely based on their serial
   * number in ascending order SHOUlD NOT BE USED ON ALIASES WHOSE SERIAL
   * NUMBERS ARE INVALID
   */
...
protected:
   ...
   *
   * DeriveDepth() MUST be called on this expression tree whenever the structure
   * of the tree is modified. -1 indicates that the depth has not been set, but
   * we have no safeguard for maintaining accurate depths between tree
   * modifications.
   */
  int depth_ = -1;

  /**
   * MUTABLE Flag indicating if there's a sub-query in the current expression or
   * in any of its children. Per LING, this is required to detect the query
   * predicate IsSupportedConjunctivePredicate.
   */
   ...
   * It is used by the QueryToOperatorTransformer to convert subquery to the
   * selected column in the sub-select
  ...
  /**
   * Copy constructs an abstract expression. Does not do a deep copy of the
   * children
   ...
   */
  void SetExpressionName(std::string expression_name) {
    expression_name_ = std::move(expression_name);
  }
  ...
  /**
   * Copies the mutable state of copy_expr. This should only be used for copying
   * where we don't need to re-derive the expression.
...
public:
   ...
   * Tianyu: No need for deep copy if your objects are always immutable! (why
   * even copy at all, but that's beyond me) Wan: objects are becoming mutable
   * again..
   */
   ...
   * DeriveSubqueryFlag() MUST be called between modifications to this
   * expression or its children for this function to return a meaningful value.
   * */
...
/**
 * To deserialize JSON expressions, we need to maintain a separate vector of all
 * the unique pointers to expressions that were created but not owned by
...
struct JSONDeserializeExprIntermediate {
  ...
  /**
   * Non-owned expressions that were created during deserialization that are
   * contained inside the abstract expression.
   */
...
/**
 * Implements std::hash for AliasTypes. This does not include the serial number
 * as we wish for parser::AliasType("xxx",1) and parser::AliasType("xxx") to be
...
template <> struct hash<db7::parser::AliasType> {
   ...
   */
  size_t operator()(const db7::parser::AliasType &p) const {
