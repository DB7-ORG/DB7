...
private:
  ...
  /** OID of the database */
  catalog::db_oid_t database_oid_ = catalog::INVALID_OID;

  /** OID of the table */
  catalog::rel_oid_t table_oid_ = catalog::INVALID_OID;

  /** OID of the column */
  catalog::col_oid_t column_oid_ = catalog::INVALID_OID;
...
public:
  ...
  ColumnValueExpression(AliasType table_alias, std::string col_name, AliasType alias)
      : AbstractExpression(ExpressionType::COLUMN_VALUE, type_id::INVALID, std::move(alias), {}),
  ...
  ColumnValueExpression(AliasType table_alias, std::string col_name)
      : AbstractExpression(ExpressionType::COLUMN_VALUE, type_id::INVALID, {}),
        table_alias_(std::move(table_alias)), column_name_(std::move(col_name)) {}
   ...
   */
  ColumnValueExpression(catalog::rel_oid_t table_oid, catalog::col_oid_t column_oid, type_id type)
   ...
   * CTEs for LogicalQueryDerivedGet's below it to reference aliases.
   ...
   */
  ColumnValueExpression(std::string table_name, std::string col_name, type_id type, AliasType alias,
                        catalog::col_oid_t column_oid)
   ...
   * @param column_oid Oid of the column (it should be a temp oid in this case)
   */
  ColumnValueExpression(AliasType table_alias, std::string col_name, type_id type, AliasType alias,
                        catalog::col_oid_t column_oid)
      : AbstractExpression(ExpressionType::COLUMN_VALUE, type, std::move(alias), {}),
        table_alias_(std::move(table_alias)), column_name_(std::move(col_name)),
        column_oid_(column_oid) {}
  ...
  ColumnValueExpression(AliasType table_alias, std::string col_name, catalog::db_oid_t database_oid,
                        catalog::rel_oid_t table_oid, catalog::col_oid_t column_oid, type_id type)
  ...
  /** @return table oid */
  catalog::rel_oid_t GetTableOid() const { return table_oid_; }
