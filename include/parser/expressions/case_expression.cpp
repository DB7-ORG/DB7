...
CaseExpression::WhenClause::FromJson(const nlohmann::json &j) {
  std::vector<std::unique_ptr<AbstractExpression>> exprs;
  auto deserialized_cond = DeserializeExpression(j.at("condition"));
  condition_ = std::move(deserialized_cond.result_);
  exprs.insert(
      exprs.end(),
      std::make_move_iterator(deserialized_cond.non_owned_exprs_.begin()),
  ...
  exprs.insert(
      exprs.end(),
      std::make_move_iterator(deserialized_then.non_owned_exprs_.begin()),
...
std::unique_ptr<AbstractExpression> CaseExpression::Copy() const {
  ...
  for (const auto &clause : when_clauses_) {
    clauses.emplace_back(
        WhenClause{clause.condition_->Copy(), clause.then_->Copy()});
  }
  auto expr = std::make_unique<CaseExpression>(
      GetReturnValueType(), std::move(clauses), default_expr_->Copy());
...
std::vector<std::unique_ptr<AbstractExpression>>
CaseExpression::FromJson(const nlohmann::json &j) {
  std::vector<std::unique_ptr<AbstractExpression>> exprs;
  ...
  exprs.insert(exprs.end(),
               std::make_move_iterator(e2.non_owned_exprs_.begin()),
