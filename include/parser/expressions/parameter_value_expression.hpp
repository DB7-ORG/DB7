 ...
 * TODO(WAN): give an example. I believe this is 0-indexed, look at
 * ParamRefTransform code path. Good beginner task?
 */
class ParameterValueExpression : public AbstractExpression {
private:
  // TODO(Tianyu): Can we get a better name for this?
  /** Offset of the value that this expression points to in the query's
   * parameter list. */
  uint32_t value_idx_;
...
public:
   ...
   * @warning we set the type to INVALID as an indicator to the binder that we
   * have not visited this expression yet. After being visited by the binder,
  ...
  /**
   * Instantiates a new ParameterValueExpression with the given offset and the
   * given type
   * @param value_idx the offset of the parameter
