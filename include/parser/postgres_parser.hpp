 ...
 * Terrier parse tree. In the future, we definitely want to replace this with
 * our own parser.
 */
...
private:
  ...
  /**
   * Transforms a single node in the parse list into a noisepage SQLStatement
   * object.
   * @param[in,out] parse_result the current parse result, which will be updated
  ...
  // *parse_result, DeleteStmt *root);

  // // DROP statements
  // static std::unique_ptr<DropStatement> DropTransform(ParseResult
  // *parse_result, DropStmt *root); static std::unique_ptr<DropStatement>
  // DropDatabaseTransform(ParseResult *parse_result, DropDatabaseStmt *root);
  // static std::unique_ptr<DropStatement> DropIndexTransform(ParseResult
  // *parse_result, DropStmt *root); static std::unique_ptr<DropStatement>
  // DropSchemaTransform(ParseResult *parse_result, DropStmt *root); static
  // std::unique_ptr<DropStatement> DropTableTransform(ParseResult
  // *parse_result, DropStmt *root); static std::unique_ptr<DropStatement>
  // DropTriggerTransform(ParseResult *parse_result, DropStmt *root);

  // // EXECUTE statements
  // static std::unique_ptr<ExecuteStatement> ExecuteTransform(ParseResult
  ...
  // // EXPLAIN statements
  // static std::unique_ptr<ExplainStatement> ExplainTransform(ParseResult
  // *parse_result, ExplainStmt *root);

  // // INSERT statements
  // static std::unique_ptr<InsertStatement> InsertTransform(ParseResult
  ...
  // // PREPARE statements
  // static std::unique_ptr<PrepareStatement> PrepareTransform(ParseResult
  // *parse_result, PrepareStmt *root);

  // static std::unique_ptr<DeleteStatement> TruncateTransform(ParseResult
  // *parse_result, TruncateStmt *truncate_stmt);

  // /**
  //  * Converts a TRANSACTION statement from postgres parser form to internal
  //  form
  //  *
  //  * @param transaction_stmt from the postgres parser
  //  * @return converted to parser::TransactionStatement
  //  */
  // static std::unique_ptr<TransactionStatement>
  // TransactionTransform(TransactionStmt *transaction_stmt);

  // // VACUUM statements as ANALYZE statements
  // static std::unique_ptr<AnalyzeStatement> VacuumTransform(ParseResult
  // *parse_result, VacuumStmt *root);

  // // VARIABLE SET statements
  // static std::unique_ptr<VariableSetStatement>
  // VariableSetTransform(ParseResult *parse_result, VariableSetStmt *root);
  // // VARIABLE SHOW statements
  // static std::unique_ptr<VariableShowStatement>
  // VariableShowTransform(ParseResult *parse_result,
  //                                                                     VariableShowStmt
  //                                                                     *root);
  ...
  UpdateTargetTransform(ParseResult *parse_result, List *root);
