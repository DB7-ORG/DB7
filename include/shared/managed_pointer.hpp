 ...
 * around the pointer to denote that the holder of this pointer has no control
 * over the life cycle of the underlying object and should not attempt to delete
 * it.
 *
 * In practice, this wrapper looks and smells like a pointer and has zero
 * overhead. It serves as a compiler hint to disallow the use of delete keyword
 * and avoid mixing of pointers to objects with different life cycles.
 *
...
public:
   ...
   * ManagedPointer<Underlying>(other)
   * @param other the other pointer to be compared with
   * @return true if the underlying pointer is equal to the given pointer, false
   * otherwise.
   */
   ...
   * ManagedPointer<Underlying>(other)
   * @param other the other pointer to be compared with
   * @return true if the underlying pointer is not equal to the given pointer,
   * false otherwise.
   */
  bool operator!=(Underlying *other) const { return underlying_ != other; }
  ...
  /**
   * Performs a reinterpret cast on the underlying pointer of a ManagedPointer
   * to a different type
   * @tparam NewType type to cast to. Underlying type must be reinterpretable to
   * new type
  ...
  template <class NewType> ManagedPointer<NewType> CastManagedPointerTo() const {
    ...
    static_assert(std::is_convertible_v<Underlying *, NewType *> ||
                  std::is_base_of_v<Underlying, NewType>);
