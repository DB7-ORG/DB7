...
public:
  ...
  static bool IsEmpty() { return state_size_ == 0; }
};

constexpr u64 MAX_OPTIMISTIC_TRIES = 1;

enum class LockMode { None, Optimistic, Read, Write };
 ...
 *      tried several times (MAX_OPTIMISTIC_TRIES) to optimistically read a
 * node.
 * - Optimistic (LockMode::Optimistic)
 *      Only taken by readers or writers while they are descending down the
 * tree. Does not aquire any locks just reads a version field and hopes that
 * version doesnt change. In case of reading a version while someone is holding
 * an exclusive lock or at the end of the node scan it reads some different
 * version then it increments tl_tries and tries again. If it fails to read
 * optimistically for couple of runs then there is a fallback to SharedLock.
 * - None (LockMode::None)
