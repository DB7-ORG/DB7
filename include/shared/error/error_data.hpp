...
#include "shared/error/error_code.hpp"
#include "shared/error/error_defs.hpp"
#include "shared/macro_helper.hpp"

namespace db7::shared {

/**
 * Class to maintain error state when they occur in the system. These can be a
 * variety of levels (@see ErrorSeverity). They contain a number of fields (@see
 * ErrorField), but only two are required: message and code (@see ErrorCode).
 * These are meant to provide verbose error output to the client (@see
 * PostgresPacketWriter::WriteError), and are not intended to be relied on for
 * control flow.
 */
class ErrorData {
public:
  /**
   * Don't need a default constructor, we want to force some fields to be
  ...
  /**
   * @param severity how serious is this error. Severity determines if it will
   * be written back to the client as an Error or a Notice message. @see
   * PostgresPacketWriter::WriteError
