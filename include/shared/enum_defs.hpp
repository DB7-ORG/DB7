...
/** Used in defining EnumTypeFromString(). Hardcoded to expect str to be in
 * scope. */
 ...
 *
 * The purpose of this macro is to keep the ToString and FromString defined and
 * in-sync. The string version of the enum is the same as the enum value itself.
 */
#define ENUM_DEFINE(EnumName, EnumCppType, EnumValMacro)                       \
  enum class EnumName : EnumCppType {                                          \
    EnumValMacro(ENUM_DEFINE_ITEM) NUM_ENUM_ENTRIES                            \
  };                                                                           \
                                                                               \
  /** @return String version of @p type. */                                    \
  inline std::string EnumName##ToString(                                       \
      EnumName type) { /* NOLINT inline usage */                               \
    switch (type) {                                                            \
      EnumValMacro(ENUM_TO_STRING);                                            \
    default: {                                                                 \
      throw std::runtime_error(("No enum conversion for: " +                   \
  ...
  inline EnumName EnumName##FromString(                                        \
      const std::string &str) { /* NOLINT inline usage */                      \
    if (false) { /* This check starts an if-else chain for the macro. */       \
    }                                                                          \
    EnumValMacro(                                                              \
        ENUM_FROM_STRING) else { /* NOLINT readability/braces confusion */     \
