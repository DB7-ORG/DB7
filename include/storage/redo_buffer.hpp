...
private:
  ...
  // Flag to denote if this RedoBuffer has flushed records to the log manager
  // already. We use this to determine if we should write an abort record, since
  // we only need to write an abort record if this buffer has previously flushed
  // logs to the log manager. In the case of recovery, the abort record helps it
  // discard changes from aborted txns
  bool has_flushed_;
...
public:
  ...
  ~RedoBuffer() { pool_->Release(segment_); }
