...
void prep_keys(std::vector<access::DataChunk *> &strs, u32 n) {
  ...
  for (int i = 0; i < int(n); i++) {
    ...
    for (int j = 0; j < int(types.size()); j++) {
      char num[11];
      std::snprintf(num, sizeof(num), "%05u",
                    i); // zero-padded: sorts correctly
...
//         auto btree = db7::access::BTreeIndex<TupleId>(&buffer_pool,
//         &disk_mng_async, 103, {{1, phys_type}});
...
//             DB7_ASSERT(v.success, "all keys present after concurrent
//             insert"); DB7_ASSERT(std::ranges::find(res_vec.vec, TupleId{1000
//             + i}) != res_vec.vec.end(),
...
//                 auto v = btree.Get(strs[i], res_vec);
//                 DB7_ASSERT(v.success, "all keys present after concurrent
//                 insert"); DB7_ASSERT(std::ranges::find(res_vec.vec,
//                 TupleId{1000 + i}) != res_vec.vec.end(),
//                         "value 1000+i present after concurrent insert");
//             } });
//         sum_insert += ins_ns;
...
//         //         DB7_ASSERT(v.success, "all keys present after concurrent
//         insert");
//         //         DB7_ASSERT(std::ranges::find(res_vec.vec, 1000 + i) !=
//         res_vec.vec.end(),
//         //             "value 1000+i present after concurrent insert");
//         //     } });
