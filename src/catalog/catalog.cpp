...
ResultObj<db_oid_t> Catalog::CreateDatabase(transaction::TransactionContext *txn,
...
bool Catalog::CreateDatabaseEntry(transaction::TransactionContext *txn, const std::span<byte> name,
                                  db_oid_t oid) {
  ...
  if (!res_name.success) { return false; }

  auto res_oid = databases_index_datoid->InsertUnique(txn, chunk, tup);
  if (!res_oid.success) { return false; }
...
bool Catalog::DeleteDatabaseEntry(transaction::TransactionContext *txn, db_oid_t oid) {
  ...
  if (!databases_->DeleteUndoRaw(txn, tup_id)) { return false; }

  return true;
}

bool Catalog::UpdateDatabaseName(transaction::TransactionContext *txn, db_oid_t oid,
                                 std::span<byte> name) {
  if (!DeleteDatabaseEntry(txn, oid)) { return false; }

  if (!CreateDatabaseEntry(txn, name, oid)) { return false; }
