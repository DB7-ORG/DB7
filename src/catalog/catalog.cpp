...
ResultObj<db_oid_t>
Catalog::CreateDatabase(transaction::TransactionContext *txn,
  ...
  DatabaseCatalog *dbc =
      Builder::CreateDatabaseCatalog(buffer_pool_, disk_mng_, oid);
...
bool Catalog::CreateDatabaseEntry(transaction::TransactionContext *txn,
...
bool Catalog::DeleteDatabase(transaction::TransactionContext *txn,
                             const db_oid_t oid) {
...
bool Catalog::DeleteDatabaseEntry(transaction::TransactionContext *txn,
                                  db_oid_t oid) {
  ...
  ResultObj<TupleId> res =
      txn->GetTidForModify(tids.vec, databases_->GetTableOid());
  if (!res.success) {
    return false;
  }
...
bool Catalog::UpdateDatabaseName(transaction::TransactionContext *txn,
                                 db_oid_t oid, std::span<byte> name) {
  if (!DeleteDatabaseEntry(txn, oid)) {
    return false;
  }
