...
#include "catalog/catalog_defs.hpp"
#include "parser/expressions/abstract_expression.hpp"
...
namespace noisepage::catalog
{
    ...
    public:
        ...
        {
            assert(options_.find(option) == options_.end() && "Adding duplicate option to INDEX options");
        ...
        bool operator==(const IndexOptions &other) const
        {
            ...
            for (const auto &pair : options_)
                current_options.insert(pair.first);
            for (const auto &pair : other.options_)
                other_options.insert(pair.first);
            if (current_options != other_options)
                return false;
        ...
        void FromCatalogString(const std::string &options)
        {
            ...
            for (auto &token : tokens)
            {
                ...
                if (pos != std::string::npos)
                {
                    ...
                    assert(val != UNKNOWN && "Invalid IndexOptions::Knob serialized");
    ...
    public:
        ...
        public:
            ...
            hash_t Hash() const
            {
                ...
                if (definition_ != nullptr)
                    hash = common::HashUtil::CombineHashes(hash, definition_->Hash());
                return hash;
            }
            ...
            bool operator==(const Column &rhs) const
            {
                ...
                if (type_ != rhs.type_)
                    return false;
                if (attr_length_ != rhs.attr_length_)
                    return false;
                if ((ShouldHaveTypeModifier() != rhs.ShouldHaveTypeModifier()) ||
                    ...
                    return false;
                if (nullable_ != rhs.nullable_)
                    return false;
                if (oid_ != rhs.oid_)
                    return false;
                if (definition_ == nullptr)
                    return rhs.definition_ == nullptr;
        ...
        private:
            ...
            void Validate() const
            {
                assert(type_ != execution::sql::SqlTypeId::Invalid && "Attribute type cannot be INVALID.");
                assert(definition_ != nullptr && "Definition cannot be nullptr.");

                if (type_ == execution::sql::SqlTypeId::Varchar || type_ == execution::sql::SqlTypeId::Varbinary)
                {
                    assert(attr_length_ == storage::VARLEN_COLUMN && "Invalid attribute length.");
                    // TODO(Matt): uncomment this assertion once we decide what a reasonable default is for unlimited varlens

                    //        assert(type_modifier_ == -1 || type_modifier_ > 0,
                    //                         "Type modifier should be -1 (no limit), or a positive integer.");
                }
                else if (type_ == execution::sql::SqlTypeId::Decimal)
                {
                    assert(attr_length_ == 16 && "Invalid attribute length.");
                    assert(type_modifier_ > 0 && "Type modifier should be a  positive integer.");
                }
                else
                {
                    assert((attr_length_ == 1 || attr_length_ == 2 || attr_length_ == 4 || attr_length_ == 8) &&
                           "Invalid attribute length.");
                    assert(type_modifier_ == -1 && "Invalid attribute modifier. Should be -1 for types that don't use it.");
                }
            }
        ...
        {
            assert(((is_primary && is_unique) || (!is_primary)) && "is_primary requires is_unique to be true as well.");
        ...
        const Column &GetColumn(const std::string &name) const
        {
            ...
            // TODO(John): Should this be a assert to have the same semantics
        ...
        void ExtractIndexedColOids()
        {
            ...
            {
                assert(col.StoredExpression() != nullptr && "Index column expr should not be missing");
                ...
                while (!expr_queue.empty())
                {
                    ...
                    {
                        assert(child != nullptr && "We should not be adding missing expressions to the queue");
        ...
        bool operator==(const IndexSchema &rhs) const
        {
            ...
            if (is_primary_ != rhs.is_primary_)
                return false;
            if (is_exclusion_ != rhs.is_exclusion_)
                return false;
            if (is_immediate_ != rhs.is_immediate_)
                return false;
            // TODO(Ling): Does column order matter for compare equal?
            if (indexed_oids_ != rhs.indexed_oids_)
                return false;
