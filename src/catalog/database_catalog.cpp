...
bool DatabaseCatalog::TryLock(transaction::TransactionContext *txn) {
  ...
  if (current_val == txn->FinishTime()) { return true; }
...
bool DatabaseCatalog::DeleteNamespaceEntry(transaction::TransactionContext *txn,
  ...
  if (tup_id == INVALID_TID) { return true; }

  if (!namespaces_->DeleteUndoRaw(txn, tup_id)) { return false; }
...
ResultObj<class_oid_t> DatabaseCatalog::CreateTable(transaction::TransactionContext *txn,
  ...
  if (!ns_res.success) { return ResultObj<class_oid_t>::Fail(ns_res.message); }
  ...
  for (auto col : schema) {
    ...
    if (!res_col.success) { return res_col; }
  }
...
bool DatabaseCatalog::DeleteTableEntry(transaction::TransactionContext *txn, class_oid_t oid) {
  ...
  if (!classes_->DeleteUndoRaw(txn, tup_id)) { return false; }
