...
bool DatabaseCatalog::TryLock(transaction::TransactionContext *txn) {
  ...
  if (transaction::TransactionUtil::HasConflict(current_val, txn->FinishTime(),
                                                txn->StartTime())) {
    txn->Abort();
    return false;
  }
...
DatabaseCatalog::CreateNamespaceEntry(transaction::TransactionContext *txn,
  ...
  auto res_name =
      namespaces_index_nspname_->InsertUnique(txn, chunk, tup.GetValue());
  if (!res_name.success) {
    return ResultObj<namespace_oid_t>::Fail(
        "Failed to insert to nspname index");
  }

  auto res_oid =
      namespaces_index_nspoid_->InsertUnique(txn, chunk, tup.GetValue());
  if (!res_oid.success) {
...
ResultObj<namespace_oid_t>
DatabaseCatalog::CreateNamespace(transaction::TransactionContext *txn,
                                 const std::span<byte> name) {
  if (!TryLock(txn))
...
bool DatabaseCatalog::DeleteNamespaceEntry(transaction::TransactionContext *txn,
  ...
  ResultObj<TupleId> res =
      txn->GetTidForModify(tids.vec, namespaces_->GetTableOid());
  if (!res.success) {
    return false;
  }
...
bool DatabaseCatalog::DeleteNamespace(transaction::TransactionContext *txn,
                                      namespace_oid_t oid) {
  if (!TryLock(txn))
    return false;
...
ResultObj<void>
DatabaseCatalog::ExistsNamespace(transaction::TransactionContext *txn,
                                 namespace_oid_t oid) {
...
bool DatabaseCatalog::UpdateNamespaceName(transaction::TransactionContext *txn,
                                          namespace_oid_t oid,
                                          std::span<byte> name) {
  if (!DeleteNamespaceEntry(txn, oid)) {
    return false;
...
DatabaseCatalog::CreateColumnEntry(transaction::TransactionContext *txn,
                                   class_oid_t rel_oid,
  ...
  access::DataChunkBuilder::BuildAttributeChunk(
      chunk, oid, rel_oid, column.GetNameSpan(), column.GetType(),
      column.GetTypeSize(), column.IsNullable());

  TupleId tup = attributes_->Insert(txn, chunk);

  auto res_name = attributes_index_attrelid_attname_->InsertUnique(
      txn, chunk, tup.GetValue());
  if (!res_name.success) {
    return ResultObj<attribute_oid_t>::Fail(
        "Failed to insert to attrelid_attname index");
  }

  auto res_oid =
      attributes_index_attnum_->InsertUnique(txn, chunk, tup.GetValue());
  if (!res_oid.success) {
...
DatabaseCatalog::CreateTableEntry(transaction::TransactionContext *txn,
  ...
  access::DataChunkBuilder::BuildClassChunk(chunk, oid, name, namespace_oid,
                                            ToChar(kind), name);

  TupleId tup = classes_->Insert(txn, chunk);

  auto res_relname =
      classes_index_relname_->InsertUnique(txn, chunk, tup.GetValue());
  ...
  if (!res_name.success) {
    return ResultObj<class_oid_t>::Fail(
        "Failed to insert to relnamespace index");
  }

  auto res_oid =
      classes_index_reloid_->InsertUnique(txn, chunk, tup.GetValue());
  if (!res_oid.success) {
...
ResultObj<class_oid_t> DatabaseCatalog::CreateTable(
    transaction::TransactionContext *txn, const std::span<byte> name,
  ...
  auto res =
      CreateTableEntry(txn, name, oid, namespace_oid, RelKind::REGULAR_TABLE);
  if (!res.success) {
    return res;
  }
...
/**
 * identical to CreateTable hard to generalize this since they are threated
 * differently for example u use ALTER TABLE to rename table and ALTER INDEX to
...
bool DatabaseCatalog::DeleteTableEntry(transaction::TransactionContext *txn,
                                       class_oid_t oid) {
  ...
  ResultObj<TupleId> res =
      txn->GetTidForModify(tids.vec, classes_->GetTableOid());
  if (!res.success) {
    return false;
  }
...
ResultObj<void>
DatabaseCatalog::ExistsTable(transaction::TransactionContext *txn,
                             class_oid_t oid) {
  auto *chunk = classes_data_chunk_layout_->CreateDataChunk();
...
bool DatabaseCatalog::UpdateTableName(transaction::TransactionContext *txn,
                                      class_oid_t oid, std::span<byte> name,
...
ResultObj<class_oid_t> DatabaseCatalog::CreateIndexEntry(
  ...
  access::DataChunkBuilder::BuildIndexChunk(
      chunk, class_oid, rel_oid, schema.IsUnique(), schema.IsPrimary(),
      schema.IsExclusion(), schema.IsImmediate(), true, true, true,
      u8(IndexKind::BTREE));

  TupleId tup = indexes_->Insert(txn, chunk);

  auto res_relname =
      indexes_index_indoid_->InsertUnique(txn, chunk, tup.GetValue());
  if (!res_relname.success) {
...
ResultObj<class_oid_t>
DatabaseCatalog::CreateIndex(transaction::TransactionContext *txn,
  ...
  auto ind_res = CreateIndexEntry(txn, name, table_res.value, rel_oid,
...
ResultObj<class_oid_t>
DatabaseCatalog::CreateConstraintEntry(transaction::TransactionContext *txn,
  ...
  auto res_oid =
      constraints_index_conoid_->InsertUnique(txn, chunk, tup.GetValue());
  if (!res_oid.success) {
    return ResultObj<class_oid_t>::Fail("Failed to insert to conoid index");
  }

  auto res_name =
      constraints_index_conname_->InsertUnique(txn, chunk, tup.GetValue());
  if (!res_name.success) {
  ...
  if (!res_ns.success) {
    return ResultObj<class_oid_t>::Fail(
        "Failed to insert to connamespace index");
  }
...
ResultObj<class_oid_t>
DatabaseCatalog::CreateConstraint(transaction::TransactionContext *txn,
                                  ConstraintProps props) {
  if (!TryLock(txn))
    return INVALID_OID;
...
void Display(transaction::TransactionContext *txn,
             access::DataChunkLayout *data_chunk_layout, access::Table *table,
             int n = 4) {
  u32 pid = 1;
