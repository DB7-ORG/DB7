...
#include "excecution/sql/value_util.hpp"
...
namespace noisepage::parser
{
    ...
    void ConstantValueExpression::Validate() const
    {
        ...
        {
            assert(return_value_type_ == execution::sql::SqlTypeId::Boolean && "Invalid TypeId for Val type.");
        }
        else if (std::holds_alternative<execution::sql::Integer>(value_))
        {
            assert((return_value_type_ == execution::sql::SqlTypeId::TinyInt ||
        ...
        {
            assert(return_value_type_ == execution::sql::SqlTypeId::Double && "Invalid TypeId for Val type.");
        }
        else if (std::holds_alternative<execution::sql::DateVal>(value_))
        {
            assert(return_value_type_ == execution::sql::SqlTypeId::Date && "Invalid TypeId for Val type.");
        }
        else if (std::holds_alternative<execution::sql::TimestampVal>(value_))
        {
            assert(return_value_type_ == execution::sql::SqlTypeId::Timestamp && "Invalid TypeId for Val type.");
        }
        else if (std::holds_alternative<execution::sql::StringVal>(value_))
        {
            assert((return_value_type_ == execution::sql::SqlTypeId::Varchar ||
    ...
    T ConstantValueExpression::Peek() const
    {
        ...
        if constexpr (std::is_same_v<T, i8> || std::is_same_v<T, i16> || std::is_same_v<T, i32> ||
                      std::is_same_v<T, i64>)
        { // NOLINT: bugprone-suspicious-semicolon: seems like a false positive
        ...
        { // NOLINT: bugprone-suspicious-semicolon: seems like a
          // false positive
    ...
    hash_t ConstantValueExpression::Hash() const
    {
        ...
        {
            return common::HashUtil::CombineHashes(hash, common::HashUtil::Hash(Peek<i64>()));
        }
    ...
    std::vector<std::unique_ptr<AbstractExpression>> ConstantValueExpression::FromJson(const nlohmann::json &j)
    {
        ...
        if (j.find("value") != j.end())
        {
            ...
            case execution::sql::SqlTypeId::BigInt:
            {
                value_ = execution::sql::Integer(j.at("value").get<i64>());
                break;
            }
            case execution::sql::SqlTypeId::Double:
            {
            ...
            case execution::sql::SqlTypeId::Timestamp:
            {
                value_ = execution::sql::TimestampVal(execution::sql::Timestamp::FromNative(j.at("value").get<u64>()));
                break;
            }
            case execution::sql::SqlTypeId::Date:
            {
                value_ = execution::sql::DateVal(execution::sql::Date::FromNative(j.at("value").get<u32>()));
                break;
            }
            case execution::sql::SqlTypeId::Varchar:
            case execution::sql::SqlTypeId::Varbinary:
