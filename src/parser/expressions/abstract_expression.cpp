...
AbstractExpression::FromJson(const nlohmann::json &j) {
  ...
  return_value_type_ = j.at("return_value_type").get<type_id>();
...
void AbstractExpression::DeriveExpressionName() {
  ...
  // TODO(WAN): I don't understand why we need to derive an expression name at
  // all. And aliases are known early.
}
