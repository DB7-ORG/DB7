...
ValueUtil::CreateStringVal(const shared::ManagedPointer<const char> string,
...
ValueUtil::CreateStringVal(const std::string &string) {
  return CreateStringVal(shared::ManagedPointer(string.data()),
                         string.length());
}
...
std::pair<StringVal, std::unique_ptr<byte[]>>
ValueUtil::CreateStringVal(const StringVal string) {
  return CreateStringVal(shared::ManagedPointer(string.GetContent()),
                         string.GetLength());
}
