...
std::unique_ptr<AbstractExpression> AggregateExpression::CopyWithChildren(
  ...
  auto expr =
      std::make_unique<AggregateExpression>(GetExpressionType(), std::move(children), IsDistinct());
...
bool AggregateExpression::RequiresCleanup() const {
  ...
  case ExpressionType::AGGREGATE_MIN:
  case ExpressionType::AGGREGATE_SUM:
  case ExpressionType::AGGREGATE_AVG: return false;
  case ExpressionType::AGGREGATE_TOP_K:
  case ExpressionType::AGGREGATE_HISTOGRAM: return true;
  default:
    throw std::runtime_error("Not a valid aggregation expression type: " +
                             std::to_string(static_cast<int>(expr_type)));
  }
