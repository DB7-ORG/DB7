...
nlohmann::json OrderByDescription::ToJson() const {
  ...
  for (const auto &expr : exprs_) { exprs_json.emplace_back(expr->ToJson()); }
  j["exprs"] = exprs_json;
  return j;
}
...
nlohmann::json GroupByDescription::ToJson() const {
  ...
  for (const auto &col : columns_) { columns_json.emplace_back(col->ToJson()); }
...
nlohmann::json SelectStatement::ToJson() const {
  ...
  for (const auto &expr : select_) { select_json.emplace_back(expr->ToJson()); }
...
std::unique_ptr<SelectStatement> SelectStatement::Copy() {
  ...
  for (auto &ref : with_table_) { new_with_tables.push_back(ref->Copy()); }
  ...
  if (union_select_ != nullptr) {
    ...
    for (auto &ref : union_select_->with_table_) { union_new_with_tables.push_back(ref->Copy()); }
    auto union_copy = std::make_unique<SelectStatement>(
        union_select_->select_, union_select_->select_distinct_, union_select_->from_->Copy(),
        union_select_->where_,
