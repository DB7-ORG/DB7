...
typedef u32 (*decodesinglefunc)(const void *compressed, u32 idx);
...
template <u32 usedBits>
static u32 decode_single(const void *compressed, u32 idx)
{
    static_assert(usedBits > 0, "usedBits must be > 0");

    constexpr u32 mask = (1U << usedBits) - 1;

    u32 block = idx >> 8;
    auto in = (const u32 *)compressed + block * usedBits * 8;

    u32 lane = (idx >> 3) & 31;
    u32 pos = idx & 7;

    u32 bitpos = lane * usedBits;
    u32 word = bitpos >> 5;
    u32 shift = bitpos & 31;

    u32 val = in[word * 8 + pos] >> shift;

    if constexpr (32 % usedBits != 0)
    {
        constexpr u32 threshold = 32 - usedBits;

        // if (shift > 32 - usedBits)
        // {
        //     val |= in[((word + 1) << 3) + pos] << (32 - shift);
        // } //TODO benchmark what is faster

        // u32 overflow = (shift > 32 - usedBits) ? 1 : 0;
        // val |= (in[(word + overflow) * 8 + pos] << (32 - shift)) * overflow;

        u32 hi = in[(word + 1) * 8 + pos] << (32 - shift);
        val |= (shift > threshold) ? hi : 0; // TODO understand this Or use a ternary that the compiler can convert to cmov:

        // u64 combined = (u64)in[word * 8 + pos] | ((u64)in[(word + 1) * 8 + pos] << 32);
...
static u32 decode_single_invalid(const void *, u32)
{
    perror("Error: decode_single doesn't support usedBits = 0");
    exit(1);
    return 0;
}

decodesinglefunc decodeSingleFuncArr[] = {
    &decode_single_invalid,
    &decode_single<1>,
    &decode_single<2>,
    &decode_single<3>,
    &decode_single<4>,
...
/* must be contain atleast 256 items */
uint32_t *avxpack(const uint32_t *in, __m256i *out, const uint32_t number, const uint32_t bit) // TODO out of the loop
{
    auto func = avxfuncPackMaskArr[bit];
...
/* reads 256 values from "in", writes  "bit" 256-bit vectors to "out" */
/* must be contain atleast 256 items */
uint32_t *avxpackwithoutmask(const uint32_t *in, __m256i *out, const uint32_t number, const uint32_t bit) // TODO out of the loop
{
    auto func = avxfuncPackArr[bit];
    for (uint32_t i = 0; i < number / 256; ++i)
    {
        func(in + i * 256, out);
        out += bit;
...
/* must be contain atleast 256 items */
uint32_t *avxunpack(const __m256i *in, uint32_t *out, const uint32_t number, const uint32_t bit) // TODO out of the loop
{
    auto func = avxfuncUnpackArr[bit];
    for (uint32_t i = 0; i < number / 256; ++i)
    {
        func(in + i * bit, out);
        out += 256;
