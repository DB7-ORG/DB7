...
namespace noisepage::storage
{
    ...
    {
    private:
        i32 size_;                 // buffer reclaimable => sign bit is 0 or size <= InlineThreshold
        byte prefix_[sizeof(u32)]; // Explicit padding so that we can use these bits for inlined values or prefix
    ...
    public:
         ...
         */
        static VarlenEntry Create(const byte *content, u32 size, bool reclaim)
        {
            VarlenEntry result;
            if (size <= InlineThreshold())
            {
                assert(!reclaim && "can't reclaim this");
            ...
            std::memcpy(result.prefix_, content, sizeof(u32));
         ...
         * @return constructed VarlenEntry object
         */
        static VarlenEntry CreateInline(const byte *content, u32 size)
        {
         ...
         * inlined.
         */
        static constexpr u32 PrefixSize() { return sizeof(u32); }

        /**
         * @return size of the varlen value stored in this entry, in bytes.
         */
        u32 Size() const { return static_cast<u32>(INT32_MAX & size_); }
        ...
        {
            return std::string_view(reinterpret_cast<const char *>(Content()), Size());
        }
        ...
        static bool CompareEqualOrNot(const VarlenEntry &left, const VarlenEntry &right)
        {
            ...
            if (size_and_prefix_same)
            {
                ...
                else
                {
                    ...
                    // of content
                    assert(std::memcmp(left.content_, &left.prefix_, PrefixSize()) == 0 &&
                    ...
                    assert(std::memcmp(right.content_, &right.prefix_, PrefixSize()) == 0 &&
                           "The prefix should be at the beginning of the non-inlined content again. We assert this since "
                           "we're about to skip it on the real comparison.");
