# to compile manually
```sh
cd ~/DB7/src/third_party/libpg_query
find src -name '*.c' \
    ! -name 'pg_query_fingerprint_defs.c' \
    ! -name 'pg_query_fingerprint_conds.c' \
    ! -name 'pg_query_json_defs.c' \
    ! -name 'pg_query_json_conds.c' \
    ! -name 'guc-file.c' \
    ! -name 'scan.c' \
    ! -name 'pg_query_json_helper.c' \
    -exec gcc -c -fPIC -Isrc/postgres/include -I. {} +
ar rcs libpg_query.a *.o
rm -f *.o
```
