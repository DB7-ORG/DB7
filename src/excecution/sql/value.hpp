...
namespace noisepage::execution::sql
{
    ...
    { // TODO(Matt): should this be refactored to Double for consistency?
    ...
    struct StringVal : public Val
    {
        ...
        StringVal(const char *str, uint32_t len) noexcept
            : Val(false),
              ...
              val_(storage::VarlenEntry::Create(reinterpret_cast<const byte *>(str), len, false))
        {
            assert(str != nullptr && "String input cannot be NULL");
        }
        ...
        static storage::VarlenEntry CreateVarlen(const StringVal &str, bool own)
        {
            if (str.is_null_)
            {
                ...
                return noisepage::storage::VarlenEntry::CreateInline(static_cast<const byte *>(nullptr), 0);
            }
            if (str.GetLength() > storage::VarlenEntry::InlineThreshold())
            {
                ...
                return noisepage::storage::VarlenEntry::Create(reinterpret_cast<const byte *>(str.GetContent()),
                                                               str.GetLength(), false);
            }
            return noisepage::storage::VarlenEntry::CreateInline(reinterpret_cast<const byte *>(str.GetContent()),
                                                                 str.GetLength());
        }
