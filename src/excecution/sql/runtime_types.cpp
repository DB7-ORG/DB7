...
#define TS_ERROR throw std::runtime_error(fmt::format("{} is not a valid timestamp", std::string(str, len)));
#define DATE_ERROR throw std::runtime_error(fmt::format("{} is not a valid date", std::string(str, len)));
    ...
    Date Date::FromYMD(int32_t year, int32_t month, int32_t day)
    {
        ...
        {
            throw std::runtime_error(fmt::format("{}-{}-{} is not a valid date", year, month, day));
        }
                                        ...
                                        int32_t sec, int32_t milli, int32_t micro, const char *ptr, const char *limit)
    {
        ...
        while (true)
        {
            ...
            {
                throw std::runtime_error(fmt::format("invalid timezone"));
            }
        }
        ...
        if (sign)
        {
            ...
            if (timezone_diff > 14 || timezone_diff < 0)
                throw std::runtime_error(fmt::format("timezone +{} out of range", timezone_diff));
    ...
    Timestamp Timestamp::FromYMDHMS(int32_t year, int32_t month, int32_t day, int32_t hour, int32_t min, int32_t sec)
    {
        ...
        {
            throw std::runtime_error(fmt::format("date field {}-{}-{} out of range", year, month, day));
        }
        ...
        {
            throw std::runtime_error(fmt::format("time field {}:{}:{} out of range", hour, min, sec));
        }
                                      ...
                                      int32_t milli, int32_t micro)
    {
        ...
        {
            throw std::runtime_error(fmt::format("time field {}:{}:{}.{}{} out of range", hour, min, sec, milli, micro));
        }
        ...
        if ((result - time) / K_MICRO_SECONDS_PER_DAY != date)
        {
            throw std::runtime_error(fmt::format("timestamp out of range {}-{}-{} {}:{}:{}.{}{} out of range", year, month,
