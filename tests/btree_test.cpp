...
Row BaseRow() { return Row{true, int32_t{4}, std::string("Jovan123!!"), 3.56}; }
...
protected:
  ...
  void ExpectAbsent(const Row &row) { EXPECT_TRUE(Lookup(row).empty()); }
};
...
TEST_F(BTreeTest, LookupOnEmptyTreeFindsNothing) { ExpectAbsent(BaseRow()); }
  ...
  RunParallel(T, [&](unsigned t) {
    ...
    for (size_t n = 0; n < kN; ++n) {
      ...
      if (rids.size() != 1 || rids[0] != RidFor(i)) {
        log.Failf("key ", i, " returned ", rids.size(), " rids");
      }
    }
  });
...
TEST_F(BTreeConcurrencyTest, RepeatedSmallConcurrentInsert) {
  ...
  Repeat(kIterations, [&](int iteration) {
    TearDown();
    SetUp();
    ...
    RunParallel(T, [&](unsigned t) {
      ...
      for (size_t i = r.lo; i < r.hi; ++i) {
        if (!Insert(NthRow(i), RidFor(i))) {
          log.Failf("iter ", iteration, ": insert failed for key ", i);
        }
      }
    });
